...
async function handleFiles(files){
    ...
    popupErrorMessage('Success',true);
}
...
async function give_accsess(file_id, username){
    ...
    // window.open('/file/'+file_id, '_blank');
    $('.access_link').css('display','block');
    $("#link_get").val(window.location.origin+'/file/'+file_id);
    // popupErrorMessage("Link Created");
}


async function get_all_in_trash(){
    const data = await $.ajax({

        type : "POST",
        mode    : "no-cors",
        url    : "/api/get_all_in_trash",
        dataType: "json",
        headers : {"Access-Control-Allow-Origin" : "*"},
        contentType: "application/json; charset=utf-8",
        data   : JSON.stringify({"token":USER_TOKEN}),
...
async function deleteFile(file_id){
    const data = await $.ajax({
        ...
        headers : {"Access-Control-Allow-Origin" : "*"},
        contentType: "application/json; charset=utf-8",
        data   : JSON.stringify({"token":USER_TOKEN,'file_id':file_id}),
        success: function(data){
            return data;
        },
